                    ...
                    <div style={{padding:'10px'}}>
                        {recentProjects.map(p => (
                            <div key={p.id} onClick={() => onOpenProject(p)} style={{padding:'10px', display:'flex', alignItems:'center', gap:'10px', cursor:'pointer', borderRadius:'6px', transition:'background 0.2s'}} onMouseOver={e=>e.currentTarget.style.background='#f5f5f5'} onMouseOut={e=>e.currentTarget.style.background='white'}>
                                <div style={{width:'30px', height:'30px', background: 'var(--primary)', borderRadius:'6px', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:'14px'}}>{p.name.charAt(0).toUpperCase()}</div>
        ...
        <div style={{padding:'30px', height:'100%', display:'flex', flexDirection:'column'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
                <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                    <div style={{width:'40px', height:'40px', background:'var(--primary)', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:'18px', fontWeight:'bold'}}>{project.name.charAt(0)}</div>
                    <div>
...
export default function App() {
  ...
  // CORRECTION MAJEURE ICI : On récupère aussi l'utilisateur du localStorage
  ...
  // LOGIN : On sauvegarde Token ET User
  const handleLogin = (tok, usr) => { 
      setToken(tok); 
      setUser(usr); 
      localStorage.setItem('hotel_token', tok);
      localStorage.setItem('hotel_user', JSON.stringify(usr)); // <--- SAUVEGARDE !
  };
  ...
  // SECURITE ANTI-PAGE BLANCHE : Si token existe mais pas user, on déconnecte
  if (token && !user) {
      handleLogout();
  ...
  return (
    <div style={{display:'flex', height:'100vh', width:'100vw'}}>
        <div className="sidebar" style={{width:'240px', flexShrink:0}}>
            <div className="sidebar-header"><span style={{background:'#f06a6a', width:'24px', height:'24px', borderRadius:'6px'}}></span>MedinaOS</div>
                    ...
                    <div key={site.id}>
                        ...
                        {siteProjects.map(proj => (
                            <div key={proj.id} className={`nav-item ${activeTab===`project-${proj.id}`?'active':''}`} onClick={() => navToProject(proj)}>
                                <span style={{width:'8px', height:'8px', borderRadius:'50%', background: activeTab===`project-${proj.id}`?'var(--primary)':'#666'}}></span>{proj.name}
                            </div>
                        ))}
                    </div>
                );
            })}
            <div style={{marginTop:'auto', padding:'20px'}}><button onClick={handleLogout} style={{background:'transparent', border:'1px solid #444', color:'white', width:'100%', padding:'8px', borderRadius:'6px', cursor:'pointer'}}>Déconnexion</button></div>
        </div>

        <div style={{flex:1, overflow:'hidden', background:'white'}}>
            {activeTab === 'home' && <Dashboard user={user} onOpenProject={navToProject} />}
            {activeTab === 'mytasks' && <div style={{padding:'40px'}}><h1>Mes Tâches</h1><p>Bientôt disponible...</p></div>}
            {activeTab.startsWith('project-') && selectedProject && (
                <ProjectView project={selectedProject} tasks={projectData.tasks} members={projectData.members} viewMode={viewMode} setViewMode={setViewMode} onAddTask={createTask} onEditTask={(t) => console.log("Edit", t)} onInvite={() => console.log("Invite")} user={user} />
            )}
        </div>
    </div>
  )
