    ...
    return (
        <div style={{padding:'40px', background:'#f8fafc', minHeight:'100vh'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'30px'}}>
                ...
                {currentUser.role === 'admin' && <button onClick={()=>setIsAdding(true)} style={{background:'#2563eb', color:'white', border:'none', padding:'10px 20px', borderRadius:'8px', cursor:'pointer', fontWeight:'bold'}}>+ Ajouter un membre</button>}
            </div>
            {isAdding && (
...
function Dashboard({ projects, tasks, user, onOpenProject, allUsers }) {
            ...
            <div style={{padding:'40px', background:'#f8fafc', minHeight:'100vh'}}>
                ...
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:'20px', marginBottom:'30px'}}>
                    ...
                    <div style={{background:'white', padding:'25px', borderRadius:'16px', borderLeft:'6px solid #10b981', boxShadow:'0 4px 6px -1px rgba(0, 0, 0, 0.05)'}}>
                        <div style={{fontSize:'12px', color:'#64748b', fontWeight:'700'}}>AVANCEMENT GLOBAL</div>
                        <div style={{fontSize:'32px', fontWeight:'800', color:'#10b981'}}>{globalProgress}%</div>
    ...
    const myProgress = myTotal === 0 ? 0 : Math.round((myDone / myTotal) * 100);
    ...
    return (
        <div style={{display:'flex', height:'100vh', fontFamily:'"Inter", "Segoe UI", sans-serif', overflow:'hidden'}}>
            ...
            <div style={{position:isMobile?'fixed':'relative', zIndex:999, height:'100%', transform:(isMobile&&!isMenuOpen)?'translateX(-100%)':'translateX(0)', transition:'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)', boxShadow:isMobile?'5px 0 15px rgba(0,0,0,0.1)': 'none'}}>
                <Sidebar sites={sites} projects={projects} activeProject={activeProject} user={user} setActiveProject={setActiveProject} setPage={setCurrentPage} closeMobileMenu={()=>setIsMenuOpen(false)} onLogout={()=>{setUser(null); localStorage.removeItem('user');}} 
            ...
            {editingTask && <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', justifyContent:'center', alignItems:'center', padding:'20px'}}><div style={{background:'white', padding:'30px', borderRadius:'16px', width:'100%', maxWidth:'450px', display:'flex', flexDirection:'column', gap:'20px', boxShadow:'0 25px 50px -12px rgba(0, 0, 0, 0.25)'}}>
                ...
                <div style={{display:'flex', gap:'15px'}}><select value={editingTask.status} onChange={e=>setEditingTask({...editingTask, status:e.target.value})} style={{flex:1, padding:'12px', borderRadius:'8px', border:'1px solid #cbd5e1'}}><option value="todo">À faire</option><option value="doing">En cours</option><option value="done">Terminé</option></select><select value={editingTask.priority} onChange={e=>setEditingTask({...editingTask, priority:e.target.value})} style={{flex:1, padding:'12px', borderRadius:'8px', border:'1px solid #cbd5e1'}}><option value="low">Basse</option><option value="normal">Normale</option><option value="high">Haute 🔥</option></select></div>
                {user.role === 'admin' && <select value={editingTask.assignee_id || ''} onChange={e=>setEditingTask({...editingTask, assignee_id:parseInt(e.target.value)})} style={{padding:'12px', borderRadius:'8px', border:'1px solid #cbd5e1'}}><option value="">-- Assigner à --</option>{allUsers.map(u => <option key={u.id} value={u.id}>{u.username}</option>)}</select>}
                <label style={{fontSize:'12px', color:'#64748b', marginBottom:'-15px'}}>Date d'échéance</label><input type="date" value={editingTask.due_date ? editingTask.due_date.split('T')[0] : ''} onChange={e=>setEditingTask({...editingTask, due_date:e.target.value})} style={{padding:'12px', border:'1px solid #cbd5e1', borderRadius:'8px'}} />
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'10px'}}><button onClick={async ()=>{ if(confirm("Supprimer ?")) { await fetch(`${API_BASE}/tasks/${editingTask.id}`, { method:'DELETE' }); fetchData(); setEditingTask(null); }}} style={{color:'#ef4444', background:'none', border:'none', cursor:'pointer', fontWeight:'bold', fontSize:'13px'}}>🗑️ Supprimer</button><div style={{display:'flex', gap:'10px'}}><button onClick={()=>setEditingTask(null)} style={{padding:'10px 20px', border:'none', background:'#f1f5f9', color:'#64748b', borderRadius:'8px', cursor:'pointer', fontWeight:'600'}}>Annuler</button><button onClick={async ()=>{ await fetch(`${API_BASE}/tasks/${editingTask.id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(editingTask)}); fetchData(); setEditingTask(null); }} style={{padding:'10px 20px', background:'#2563eb', color:'white', border:'none', borderRadius:'8px', cursor:'pointer', fontWeight:'600'}}>Enregistrer</button></div></div>
            </div></div>}
        </div>
    );
}
