...
function MembersView({ users, currentUser, onAddUser, onDeleteUser }) {
    ...
    const handleSubmit = (e) => {
        ...
        onAddUser(newUser); setIsAdding(false); setNewUser({ username: '', email: '', password: '', role: 'user' });
    };

    return (
        <div style={{padding:'40px', background:'#f8fafc', minHeight:'100vh'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'30px'}}>
                <div><h1 style={{fontSize:'24px', color:'#1e293b', marginBottom:'5px'}}>👥 L'Équipe Medina</h1><p style={{color:'#64748b'}}>Gérez les accès à la plateforme.</p></div>
                {currentUser.role === 'admin' && <button onClick={()=>setIsAdding(true)} style={{background:'#2563eb', color:'white', border:'none', padding:'10px 20px', borderRadius:'8px', cursor:'pointer', fontWeight:'bold'}}>+ Ajouter</button>}
            </div>
            {isAdding && (
                <div style={{background:'white', padding:'25px', borderRadius:'12px', marginBottom:'30px', border:'1px solid #e2e8f0'}}>
                    ...
                    <form onSubmit={handleSubmit} style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'15px'}}>
                        <input placeholder="Nom" value={newUser.username} onChange={e=>setNewUser({...newUser, username:e.target.value})} style={{padding:'10px', border:'1px solid #cbd5e1', borderRadius:'6px'}} />
                        ...
                        <input placeholder="Mot de passe" type="password" value={newUser.password} onChange={e=>setNewUser({...newUser, password:e.target.value})} style={{padding:'10px', border:'1px solid #cbd5e1', borderRadius:'6px'}} />
                        <select value={newUser.role} onChange={e=>setNewUser({...newUser, role:e.target.value})} style={{padding:'10px', border:'1px solid #cbd5e1', borderRadius:'6px'}}><option value="user">Collaborateur</option><option value="admin">Administrateur</option></select>
                        <div style={{display:'flex', gap:'10px'}}><button type="button" onClick={()=>setIsAdding(false)} style={{background:'#f1f5f9', border:'none', padding:'10px', borderRadius:'6px', cursor:'pointer'}}>Annuler</button><button type="submit" style={{background:'#10b981', color:'white', border:'none', padding:'10px', borderRadius:'6px', cursor:'pointer', flex:1, fontWeight:'bold'}}>Valider</button></div>
                    </form>
                </div>
            )}
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:'20px'}}>
                {users.map(u => (
                    <div key={u.id} style={{background:'white', padding:'20px', borderRadius:'12px', boxShadow:'0 2px 5px rgba(0,0,0,0.05)', display:'flex', alignItems:'center', gap:'15px', border:'1px solid #f1f5f9'}}>
                        ...
                        <div style={{flex:1}}><div style={{fontWeight:'bold', color:'#1e293b', fontSize:'16px'}}>{u.username} {u.id === currentUser.id && '(Moi)'}</div><div style={{color: u.role==='admin'?'#ef4444':'#64748b', fontSize:'12px', textTransform:'uppercase', fontWeight:'bold'}}>{u.role==='admin'?'🛡️ Administrateur':'👤 Collaborateur'}</div><div style={{fontSize:'12px', color:'#94a3b8'}}>{u.email}</div></div>
                        {currentUser.role === 'admin' && u.id !== currentUser.id && <button onClick={() => { if(confirm("Supprimer "+u.username+" ?")) onDeleteUser(u.id); }} style={{background:'transparent', border:'none', cursor:'pointer', fontSize:'16px', opacity:0.5}}>🗑️</button>}
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div style={{padding:'40px', background:'#f8fafc', minHeight:'100vh'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(2, 1fr)', gap:'20px', marginTop:'20px'}}>
                <div style={{background:'white', padding:'20px', borderRadius:'16px', boxShadow:'0 2px 5px rgba(0,0,0,0.05)'}}><div style={{fontSize:'28px', fontWeight:'bold', color:'#3b82f6'}}>{myTasks.length}</div><div style={{fontSize:'12px', color:'#64748b', fontWeight:'bold'}}>À FAIRE</div></div>
    ...
    return ( 
        <div style={{padding:'30px', height:'100%', display:'flex', flexDirection:'column', background:'white'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
                ...
                <div style={{display:'flex', gap:'10px'}}><button onClick={exportPDF} style={{background:'white', border:'1px solid #cbd5e1', padding:'8px 15px', borderRadius:'8px', cursor:'pointer'}}>📥 PDF</button>{user.role==='admin'&&<button onClick={()=>onDeleteProject(project.id)} style={{color:'white', background:'#ef4444', border:'none', padding:'8px 15px', borderRadius:'8px', cursor:'pointer'}}>Supprimer</button>}</div>
            </div>
            <div style={{marginBottom:'20px', display:'flex', gap:'5px'}}>
                <button onClick={()=>setViewMode('board')} style={{padding:'8px 15px', borderRadius:'6px', border:'none', background:viewMode==='board'?'#e2e8f0':'transparent', fontWeight:viewMode==='board'?'bold':'normal', cursor:'pointer'}}>Kanban</button>
            ...
            {viewMode==='board' && <div style={{display:'flex', gap:'20px', overflowX:'auto', paddingBottom:'10px', height:'100%'}}>{['todo', 'doing', 'done'].map(s=>(
                <div key={s} onDragOver={e=>e.preventDefault()} onDrop={e=>{const id=e.dataTransfer.getData("id"); const t=tasks.find(x=>x.id==id); if(t) onUpdateTask({...t, status:s});}} style={{flex:'0 0 300px', background:'#f8fafc', padding:'15px', borderRadius:'12px', border:'1px solid #eee', height:'fit-content', maxHeight:'100%', overflowY:'auto'}}>
                    ...
                    {(s==='todo'||user.role==='admin')&&<form onSubmit={e=>{e.preventDefault(); onAddTask(newTask); setNewTask("");}}><input value={newTask} onChange={e=>setNewTask(e.target.value)} placeholder="+ Tâche..." style={{width:'100%', padding:'12px', marginBottom:'15px', borderRadius:'8px', border:'1px solid #e2e8f0'}}/></form>}
...
// ==========================================
// COMPOSANT 4 : MODAL ÉDITION AVEC CHAT & FICHIERS
// ==========================================
function EditTaskModal({ task, user, allUsers, onClose, onUpdate, onDelete }) {
    ...
    const handleSendComment = async (e) => {
        ...
        if (file) {
            ...
            reader.onload = async () => {
                fileData = reader.result; // Le gros texte bizarre qui est l'image
                fileName = file.name;
                fileType = file.type;
    ...
    const postComment = async (fData, fName, fType) => {
        ...
        try {
            ...
            setNewComment("");
            setFile(null);
        } catch(err) { alert("Erreur envoi (fichier trop gros ? Max 50Mo)"); }
    };
        ...
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', justifyContent:'center', alignItems:'center', padding:'20px'}}>
            <div style={{background:'white', width:'100%', maxWidth:'800px', height:'80vh', borderRadius:'16px', display:'flex', overflow:'hidden', boxShadow:'0 25px 50px -12px rgba(0, 0, 0, 0.25)'}}>
                ...
                <div style={{width:'40%', padding:'30px', borderRight:'1px solid #e2e8f0', overflowY:'auto', background:'#f8fafc'}}>
                    ...
                    <label style={{fontSize:'12px', fontWeight:'bold', color:'#64748b'}}>STATUT</label>
                    <select value={editedTask.status} onChange={e=>setEditedTask({...editedTask, status:e.target.value})} style={{width:'100%', padding:'10px', borderRadius:'8px', border:'1px solid #cbd5e1', marginBottom:'15px', background:'white'}}><option value="todo">À faire</option><option value="doing">En cours</option><option value="done">Terminé</option></select>
                    
                    <label style={{fontSize:'12px', fontWeight:'bold', color:'#64748b'}}>PRIORITÉ</label>
                    <select value={editedTask.priority} onChange={e=>setEditedTask({...editedTask, priority:e.target.value})} style={{width:'100%', padding:'10px', borderRadius:'8px', border:'1px solid #cbd5e1', marginBottom:'15px', background:'white'}}><option value="low">Basse</option><option value="normal">Normale</option><option value="high">Haute 🔥</option></select>
                    
                    {user.role === 'admin' && (
                        <>
                            <label style={{fontSize:'12px', fontWeight:'bold', color:'#64748b'}}>RESPONSABLE</label>
                            <select value={editedTask.assignee_id || ''} onChange={e=>setEditedTask({...editedTask, assignee_id:parseInt(e.target.value)})} style={{width:'100%', padding:'10px', borderRadius:'8px', border:'1px solid #cbd5e1', marginBottom:'15px', background:'white'}}><option value="">-- Personne --</option>{allUsers.map(u => <option key={u.id} value={u.id}>{u.username}</option>)}</select>
                        </>
                    )}

                    <label style={{fontSize:'12px', fontWeight:'bold', color:'#64748b'}}>ÉCHÉANCE</label>
                    <input type="date" value={editedTask.due_date ? editedTask.due_date.split('T')[0] : ''} onChange={e=>setEditedTask({...editedTask, due_date:e.target.value})} style={{width:'100%', padding:'10px', border:'1px solid #cbd5e1', borderRadius:'8px', background:'white', marginBottom:'30px'}} />

                    <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                        <button onClick={()=>onUpdate(editedTask)} style={{padding:'12px', background:'#2563eb', color:'white', border:'none', borderRadius:'8px', cursor:'pointer', fontWeight:'bold'}}>Enregistrer les modifications</button>
                        <div style={{display:'flex', justifyContent:'space-between'}}>
                             <button onClick={onClose} style={{padding:'10px', border:'none', background:'transparent', color:'#64748b', cursor:'pointer'}}>Fermer</button>
                             <button onClick={onDelete} style={{color:'#ef4444', background:'none', border:'none', cursor:'pointer', fontSize:'12px'}}>Supprimer la tâche</button>
                        </div>
                    </div>
                </div>

                {/* COLONNE DROITE : CHAT & FICHIERS */}
                <div style={{flex:1, display:'flex', flexDirection:'column', background:'white'}}>
                    ...
                    <div style={{flex:1, overflowY:'auto', padding:'20px', display:'flex', flexDirection:'column', gap:'15px'}}>
                        ...
                        {comments.map(c => (
                            <div key={c.id} style={{display:'flex', gap:'10px', alignItems:'flex-start'}}>
                                ...
                                <div>
                                    <div style={{fontSize:'12px', color:'#94a3b8', marginBottom:'2px'}}><b>{c.username}</b> • {new Date(c.created_at).toLocaleString()}</div>
                                    <div style={{background:'#f1f5f9', padding:'10px', borderRadius:'0 10px 10px 10px', color:'#334155', fontSize:'14px'}}>
                                        {c.content}
                                        {c.file_data && (
                                            <div style={{marginTop:'10px'}}>
                                                {c.file_type.startsWith('image/') ? (
                                                    <img src={c.file_data} alt="pj" style={{maxWidth:'100%', borderRadius:'6px', border:'1px solid #e2e8f0'}} />
                                                ) : (
                                                    <a href={c.file_data} download={c.file_name} style={{display:'flex', alignItems:'center', gap:'5px', textDecoration:'none', color:'#2563eb', background:'white', padding:'5px 10px', borderRadius:'6px', border:'1px solid #bfdbfe'}}>📎 {c.file_name}</a>
                                                )}
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div style={{padding:'20px', borderTop:'1px solid #e2e8f0', background:'#f8fafc'}}>
                        <form onSubmit={handleSendComment} style={{display:'flex', gap:'10px'}}>
                            ...
                            <button type="submit" style={{background:'#10b981', color:'white', border:'none', borderRadius:'8px', padding:'0 20px', fontWeight:'bold', cursor:'pointer'}}>Envoyer</button>
                        </form>
                    </div>
                </div>
            </div>
