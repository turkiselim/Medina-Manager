...
const EMAIL_USER = 'medinahotels.belisaire@gmail.com';  // <--- Avec des guillemets
...
const sendAssignmentEmail = async (userEmail, userName, taskTitle, projectName) => {
    ...
    const mailOptions = {
        ...
        to: userEmail,
        subject: `🔔 Nouvelle mission : ${taskTitle}`,
        html: `
            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                <h2 style="color: #2563eb;">Bonjour ${userName},</h2>
                <p>La direction vous a assigné une nouvelle tâche dans le projet <strong>${projectName}</strong>.</p>
                <div style="background: #f3f4f6; padding: 15px; border-radius: 5px; margin: 20px 0;">
                    <h3 style="margin: 0;">📌 ${taskTitle}</h3>
                </div>
                <p>Merci de vous connecter au Dashboard pour voir les détails et commencer le travail.</p>
                <a href="https://medina-app.onrender.com" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;">Accéder à MedinaOS</a>
            </div>
        `
    };
    try {
        await transporter.sendMail(mailOptions);
        console.log(`Email envoyé à ${userName}`);
    } catch (error) {
...
// TÂCHES (AVEC NOTIFICATION EMAIL)
...
app.put('/tasks/:id', async (req, res) => { 
    ...
    // 1. Récupérer l'ancienne tâche pour voir si l'assigné a changé
    ...
    // 2. Si un nouvel assigné est défini, envoyer un mail
